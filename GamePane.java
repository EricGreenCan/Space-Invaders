...
public class GamePane extends Pane {
    ...
    private void update() {
        ...
        if (keyListener.isKeyPressed("SPACE") && player.canShoot) { // modified code
        	player.getXPos();
            player.getYPos();
            random_angle = Math.floor(Math.random() *(player.spread_max - player.spread_min + 1) + player.spread_min);
            
            Bullet bullet = new Bullet(2, 96, player.getXPos()+32, player.getYPos()-16, Color.rgb(255, 255, 255, 0.5), random_angle, "bullet");
            numObjects += 1;
            getChildren().add(bullet);
            bullets.add(bullet);
            player.canShoot = false;
            Timeline delay = new Timeline(new KeyFrame(Duration.seconds(player.shootDelay), e -> player.canShoot = true)); // 0.5 second delay
            delay.play();
        }
            	...
            	for (PowerUp powerup : powerups) {
            		if (bullet.getBoundsInParent().intersects(powerup.getBoundsInParent())) {
            			if (powerup.getType() == "move_speed") {
            				...
            				System.out.print(player.shootDelay);          				
            			}
        ...
        stageLabel.setLayoutX((WIDTH - stageLabel.getWidth()) / 2);
        //stageLabel.setText("Objects: " + numObjects);
    }
